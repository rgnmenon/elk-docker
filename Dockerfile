...
# * udpated to make JRE work                                                                            *
# ******************************************************************************/

FROM centos:centos7

MAINTAINER Govindaraj Venkatesan/ Remesh Govind.
...
RUN wget --no-check-certificate --no-cookies  https://mirror.its.sfu.ca/mirror/CentOS-Third-Party/RCG/common/x86_64/jdk-8u73-linux-x64.rpm -O /tmp/jdk-8u73-linux-x64.rpm; \
...
COPY ./logstash.conf /etc/logstash/conf.d/logstash-j.conf

COPY ./7.2.2.jre8.jar /usr/java/jdk1.8.0_73/jre/lib/
